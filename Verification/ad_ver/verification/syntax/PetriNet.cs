    ...
    class PetriNet
    {
        ...
        public void petriCheck(ADNodesList adList)
        {
            ...
            while (cont)
            {
                ...
                for (int stepProhod = 0; stepProhod < 2; stepProhod++)
                {     // сначала проходим по условным, затем по остальным эл-м
                    i = 0;
                ...
                if (stepResultMasks.Count==0 && leaves.Count!=0)
                {
                    ...
                    for (int j = 0; j < originMask.Count; j++)
                    {
                        if (originMask[j].type == TOKEN)
        ...
        public static String toString(MISTAKES mistake)
        {
            switch (mistake)
            {
                ...
                case MISTAKES.DEAD_ROAD: return "Тупик";
